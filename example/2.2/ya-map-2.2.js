...
function YaMapCtrl($scope, mapApiLoad) {
	var self = this;
	mapApiLoad(function (isSuccess) {
		self.addGeoObjects = function (obj) {
			isSuccess && $scope.map.geoObjects.add(obj);
		};
		self.removeGeoObjects = function (obj) {
			isSuccess && $scope.map.geoObjects.remove(obj);
		};

		self.addControl = function (name, options) {
			isSuccess && $scope.map.controls.add(name, options);
		};
		self.getMap = function () {
			return isSuccess && $scope.map;
		};
...
angular.module('yaMap', []).
   ...
   factory('mapApiLoad', ['yaMapSettings', function (yaMapSettings) {
   	var loaded = false;
   	var callbacks = [];
   	var runCallbacks = function (isSuccess) {
   		var callback;
   	...
   	var loadUrl = '//maps.yandex.ru/' + yaMapSettings.version + '/?load=package.full&lang=' +
    			...
    			return function (scope, element, attrs) {
    				...
    				var getCenterCoordinates = function (center) {
    					...
    					if (!center) {
    						...
    						mapApiLoad(function (isSuccess) {
    							isSuccess && ymaps.geolocation.get({
    						...
    						mapApiLoad(function (isSuccess) {
    							isSuccess && ymaps.geocode(center, { results: 1 }).then(function (res) {
